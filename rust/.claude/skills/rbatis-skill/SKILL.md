...
11. [Built-in Macros](#11-built-in-macros)
    - [plus! - Extended CRUD](#plus---extended-crud-operations)
    - [make_table!](#make_table---simplified-table-construction)
...
**条件语法说明**:
- 等值条件: `value! {"id": 123}` → `where id = ?`
- LIKE条件: `value! {"name like ": "%test%"}` → `where name like ?`
- 比较条件: `value! {"id > ": 100}` → `where id > ?`
- IN条件: `value! {"id": [1, 2, 3]}` → `where id in (?, ?, ?)`
- 多条件AND: `value! {"id": 1, "name": "test"}` → `where id = ? and name = ?`

**特殊参数（select_page_by_condition）**:
- `column`: 指定查询列，如 `["id", "name"]` 或 `"*"`
- `order_by`: 排序，如 `"created_date desc"`
